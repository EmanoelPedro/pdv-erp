...
function readBootstrapError(error: unknown): string {
	...
	return 'Nao foi possivel iniciar o aplicativo.'
}
...
function registerBootErrorHandlers(): void {
	window.addEventListener('error', (event) => {
		...
		renderBootState('Nao foi possivel iniciar o PDV', description, true)
	})
...
function renderBootState(title: string, description: string, isError = false): void {
...
async function bootstrapApplication(): Promise<void> {
	...
	renderBootState('Preparando o aplicativo', 'Verificando a API e carregando o ambiente local...')
