<template>
  <div class="min-h-screen bg-zinc-100 py-3 text-zinc-950">
    ...
    >
      ...
      Sem conexão &mdash; vendas serão sincronizadas ao reconectar
    </div>
    <div class="page-shell-wide" :class="{ 'mt-9': !uiStore.isOnline }">
      ...
      >
        <div class="min-w-0">
          <h1 class="text-2xl font-semibold tracking-[-0.03em] text-zinc-950">
            {{ currentTopbar.title }}
          </h1>
        </div>
    ...
    >
      <div class="flex h-full flex-col gap-4">
        <div class="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
          ...
          <Tag
            class="mt-3"
            :severity="authStore.isOwner ? 'warn' : 'secondary'"
            :value="authStore.isOwner ? 'Proprietário' : 'Funcionário'"
          />
        </div>
...
const currentTopbar = computed(() => {
  ...
  const baseTopbar = topbarByRouteName[routeName] ?? {
